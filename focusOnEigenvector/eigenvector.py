...
def decider(row, currentCompany, currentCountry):
    if (row[3] == currentCountry and row[4] == currentCountry) and (row[0]!=currentCompany or row[6] == 'C'):
...
def calc(tup):
    ...
    df = inputdf[inputdf.apply (lambda row: decider(row, company, country),axis=1)].copy()
